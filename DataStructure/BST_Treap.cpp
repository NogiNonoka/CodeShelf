...
void print(int x) {
    ...
    cout << x << " val: " << node[x].val << " lson: " << node[x].ch[0]
         << " rson: " << node[x].ch[1] << " size: " << node[x].size
         << " cnt: " << node[x].cnt << " rand: " << node[x].rand << endl;
    print(node[x].ch[0]);
    print(node[x].ch[1]);
}
#endif
...
void insert(int& x, int val) {
    ...
    if (node[x].val == val)
        ...
        if (node[x].rand < node[node[x].ch[xs]].rand) rotate(x, xs ^ 1);
    }
    update(x);
}
...
int ranking(int val) {
    int x = root, rnk = 1;
    while (x) {
        if (node[x].val == val) return rnk + node[node[x].ch[0]].size;
        if (node[x].val < val)
