 ...
 * /CodeShelf/GraphTheory/WorkOnTree_HeavyLightDecomposition.cpp
...
struct HeavyLightDecomposition {
    ...
    void dfs1(int pre, int now) {
        ...
        for (int i = head[now]; i; i = edge[i].nxt) {
            int nxt = edge[i].v;
            if (nxt == pre) continue; // Undirected Graph (Unrooted Tree)
            dfs1(now, nxt);
            siz[now] += siz[nxt]; // val[nxt] = val[now] + edge[i].w;
            if (son[now] == 0 || siz[nxt] > siz[son[now]]) son[now] = nxt;
        }
    }
    ...
    void dfs2(int pre, int now) {
        ...
        for (int i = head[now]; i; i = edge[i].nxt) {
            int nxt = edge[i].v;
            if (nxt != son[now] && nxt != fa[now]) dfs2(nxt, nxt);
        }
    }
